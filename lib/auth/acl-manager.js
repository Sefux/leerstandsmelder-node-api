...
module.exports.setAclEntry = (resource, user, action) => {
    ...
    return mongoose.model('AclEntry').findOne({resource_key: resource, user_key: user})
        .then(function (entry) {
            if (!entry) {
                entry = {
        ...
        .then(function (entry) {
            if (entry) {
                ...
                if (entry.actions.length === 0) {
                    return mongoose.model('AclEntry').findOneAndRemove({resource_key: resource, user_key: user})
                } else {
                    return entry.save();
                }
            }
