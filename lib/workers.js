...
queue.process('createThumbs', function (job, done) {
    if (!job.data) {
        done();
    }
    var domain = require('domain').create();
    ...
    domain.run(function () { // your process function
        gm(path.resolve(__dirname + '/../assets/photos/'+ job.data.uuid))
            .resize('200', '200', '^>')
            .gravity('Center')
            .crop('200', '200')
            .write(path.resolve(__dirname + '/../assets/photos/' + job.data.uuid + '-thumb'), function (err) {
                done(err);
            });
    });
});
